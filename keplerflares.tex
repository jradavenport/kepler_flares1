...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Find every flare from every star. here we describe the code used for finding flares and empirically determining flare recovery completeness from every \Kepler light curve. 
Make tools and catalog publicly available.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Comparing to Other Flare Samples}
...
can i show Lfl/Lkep for stars from my study vs others?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Flares across mass and age}

given we have estimates of flare energies, we can do the first examination of flares as a function of age for a range of masses. 

In Figure \ref{fig:rotcolor} we present the flare power as a function of color and rotation period. this space is for the study of magnetochronology, or describing how flare rates evolve as a function of time
 we use the $(g-i)$ color space in Figure \ref{fig:rotcolor} to differentiate stars, approximately as a function of spectral type or mass, because it has a superior ability to distinguish temperature across the entire main sequence compared with the $(g-r)$ color used in several previous works \citep{covey2007,davenport2014} 
...
In Figure \ref{fig:ratecolor} we show the flare rate versus rotation period for three bins of $g-i$ color. A power law relationship between flare luminosity and rotation period is seen
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}
we have presented the largest catalog of stellar flares every recovered using the \Kepler mission. our approach is the first to empirically characterize the completeness of flare recovery for every light curve.

this dataset enables a new generation of stellar population analysis, examining a fundamental property of flare rates over stellar age for a range of stellar masses. the advent of tens of thousands of rotation periods has ushered in a new era of astrophysics for gyrochronology. we expect our ensemble study of flare energies to provide a new peek in to the magnetic lives of stars.

detailed studies about flare morphology also await, such as discussed in \citet{davenport2014b} and \citet{balona2015}. specifically we are interested in learning if the decay phases of flares follows a truly universal pattern, or if it depends on stellar properties such as surface temperature and gravity.  also interested in the complex flare morpholgoy, as discussed by \citet{davenport2015c}

in this study we have preferred the total flare power as the metric, since it is more robust against choices of flare start/stop and duration, etc. however, we have also shown how to combine the short and long cadence data to get estimates of the flare energy distribution for single stars. follow-up work will examine the slope of this distribution versus stellar mass and age, and how universal it really is. will be important for setting both the rate of the largest super flares, as well as the smallest energy flares that could affect the stellar atmosphere.

future studies include comparison of flare power to other more traditional indicators of magnetic activity, such as chromosphereic emission. this sample also big enough to search for changes in the flare power over the \Kepler mission lifetime, 4 years, which is enough to see (in the case of the Sun) an order of magnitude change in flare rates
